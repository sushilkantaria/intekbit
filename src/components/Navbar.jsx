...
const Navbar = () => {
  ...
  return (
    <nav className="w-full fixed top-0 left-0 z-40 bg-transparent">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-2 sm:px-4 md:px-8 py-2 md:py-3 relative">
        ...
        >
          <Link to="/" onClick={handleHomeClick}>
            <img
              src="/assets/logos/footer-logo.webp"
              alt="Intekbitlogo"
        ...
        {/* Mobile Menu Button */}
        <button
          className="md:hidden ml-2 sm:ml-4 text-3xl sm:text-3xl text-blue-500"
          onClick={toggleMenu}
      ...
      {isMenuOpen && (
        <ul
          className="md:hidden w-full flex flex-col gap-2 sm:gap-4 p-4 z-50 rounded-2xl animate-fadeIn backdrop-blur-2xl backdrop-saturate-150 items-center shadow-[0_2px_12px_0_rgba(30,58,138,0.18),0_1px_8px_0_rgba(136,19,55,0.13),0_1px_6px_0_rgba(202,138,4,0.10),0_1px_4px_0_rgba(6,78,59,0.10),inset_0_2px_16px_0_rgba(255,255,255,0.13),inset_0_0_12px_2px_rgba(255,255,255,0.10),inset_0_0_0_1px_rgba(255,255,255,0.07)] relative group"
          data-aos="fade-down"
          data-aos-duration="1500"
        >
          <li className="w-full flex justify-center">
            <Link
              to="/"
              className="block w-full text-center py-2 rounded-lg font-bold text-lg text-blue-500"
              onClick={(e) => {
                setIsMenuOpen(false);
                handleHomeClick(e);
              }}
          ...
          <li className="w-full flex flex-col items-center">
            <button
              className="w-full text-center py-2 rounded-lg font-bold text-lg text-blue-500 flex items-center justify-center gap-2 focus:outline-none"
            ...
            {isMobileServicesOpen && (
              <ul
                ...
                className="w-full flex flex-col gap-1 mt-1 p-2"
              >
                {servicesList.map((service) => (
                  <li key={service.path} className="w-full">
                    <Link
                      to={service.path}
                      className="flex items-center gap-2 px-3 py-2 text-blue-500 text-left font-bold"
                      onClick={() => {
                        setIsMenuOpen(false);
